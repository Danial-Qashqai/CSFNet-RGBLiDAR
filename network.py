...
def CSFNet(version, pretrain, backbone_path, num_classes):

      if version == "CSFNet-1":
                   backbone="STDCNet813"

      return Network(num_class=num_classes, pretrain=pretrain , backbone_path=backbone_path)



class Network(nn.Module):
    def __init__(self, num_class=19, pretrain=None ,backbone_path=None , act_type='relu'):
        super(Network, self).__init__()

        
        pool_out=[(16, 40),(8, 20),(4, 10),(2, 5),(4,12)]

        decoder_channels = [32, 64, 128, 32, num_class]

        self.encoder = Encoder(pretrain, backbone_path, pool_out)
...
class Encoder(nn.Module):
    def __init__(self, pretrain, backbone_path, pool_out):
        super(Encoder, self).__init__()

        self.encoder = STDCNet813(pretrain_model=pretrain, backbone_path=backbone_path  , pool_out=pool_out)
...
class Decoder(nn.Module):
    ...
    def forward(self, x1, x3, x4, x5):
        ...
        x = self.CBR2(x)
        x = F.interpolate(x, size=(47,176), mode='bilinear', align_corners=True)
...
class STDCNet813(nn.Module):
    def __init__(self, base=64, layers=[2, 2, 2], block_num=4, type="cat", backbone_path=None, pretrain_model=False ,pool_out=None):
        ...
        self.features_d = self._make_layersd(base, layers, block_num, block)
    ...
    def _make_layersd(self, base, layers, block_num, block):
        features = []

        features += [ConvX(2, base // 2, 3, 2)]
        features += [ConvX(base // 2, base, 3, 2)]
